...
// Base API endpoint targeting Vite/React environment variables with local fallback
const API_BASE_URL =
...
export default function CareerPrep() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/intake/apply`, {
        method: "POST",
        body: formData,
      });

      // 6. Safe JSON parsing (handles non-JSON server errors e.g. 500 HTML responses)
      let data = {};
      try {
        data = await res.json();
      } catch {
      ...
      if (!res.ok) {
        if (typeof data.detail === "string") {
          ...
          // Flatten FastAPI array validation errors
          const errorMessages = data.detail
            .map((err) => `${err.loc?.[1] || "field"}: ${err.msg}`)
            .join(", ");
          setServerError(errorMessages);
        } else {
          setServerError(`Server error (${res.status}). Please try again later.`);
        }
      } else {
        setSubmitted(true);
      }
