...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || 'Failed to reset password. Please check your reset link or try requesting a new one.';
      setError(msg);
    } finally {
      setLoading(false);
    }
        ...
        <div className="signin-form-container">
          ...
          {!token ? (
            <div style={{ textAlign: 'center', margin: '20px 0' }}>
              ...
              <Link to="/forgot-password" className="signin-submit-btn" style={{ textDecoration: 'none', margin: '0 auto', display: 'inline-flex' }}>
                Request New Link
              </Link>
            </div>
          ) : successMessage ? (
            <div style={{ textAlign: 'center', margin: '20px 0' }}>
              ...
              <Button type="button" className="signin-submit-btn" onClick={() => navigate('/signin')} style={{ margin: '0 auto' }}>
                Go to Sign In
              </Button>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-actions" style={{ marginTop: '24px' }}>
                ...
                <div className="signup-redirect" style={{ marginTop: '12px' }}>
                  Back to <Link to="/signin" style={{ textDecoration: 'underline', fontWeight: 'bold' }}>Sign In</Link>
                </div>
              </div>
            </form>
          )}
          ...
          <footer className="form-footer-notice">
            ...
            <span>Password resets are available for mentors and students</span>
          </footer>
        </div>
      </div>
    </div>
